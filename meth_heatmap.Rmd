...
#statistics
```{r}
gene_density_col <- gene_density %>% 
  mutate(genes = meth) %>% 
  select(chr, start, genes)

head(gene_density_col)
```

```{r}
heatmap_data_2 <- heatmap_data_s %>%
  filter(context != "genes") %>% 
  full_join(gene_density_col, by = c("chr", "start")) %>% 
  group_by(sample_type, genotype, context) %>% 
  mutate(bucket = ntile(genes, 50)) %>% 
  ungroup() %>% 
  mutate(bucket = factor(bucket))

head(heatmap_data_2, 10)
```
```{r}
heatmap_data_2g <- heatmap_data_2 %>% 
  group_by(context, genotype, sample_type, bucket) %>% 
  summarise(mean.gene.overlap = mean(genes)) %>% 
  inner_join(heatmap_data_2, by = c("context", "genotype", "sample_type", "bucket"))

head(heatmap_data_2g)
```
...
heatmap_data_2g %>% 
  filter(sample_type == "leaf"|
           sample_type == "egg"|
           sample_type == "sperm") %>% 
ggplot(aes(x = mean.gene.overlap * 100, y = meth)) +
  stat_summary(geom = "ribbon", 
               aes(group = sample_type, fill = sample_type), fun.data  = "mean_cl_boot", size = 1.5, alpha = 0.3) +
  stat_summary(geom = "line", aes(group = sample_type, color = sample_type), fun.y = mean, size = 1.5, alpha = 0.8) +
  facet_grid(context ~ ., scales = "free_y", switch = "y") +
  scale_color_manual(values = c("tomato1", "darkgreen", "dodgerblue2")) +
  labs(x = "% overlapped by genes", 
       y = NULL,
       fill = NULL,
       color = NULL) +
  theme_minimal() +
...
ggsave("meth vs gene.svg", height = 6, width = 4)
```
...
heatmap_data_2g %>% 
  filter(sample_type == "leaf"|
           sample_type == "egg"|
           sample_type == "sperm") %>% 
ggplot(aes(x = mean.gene.overlap * 100, y = z)) +
  stat_summary(geom = "ribbon", 
               aes(group = sample_type, fill = sample_type), fun.data  = "mean_cl_boot", size = 1.5, alpha = 0.3) +
  stat_summary(geom = "line", aes(group = sample_type, color = sample_type), fun.y = mean, size = 1.5, alpha = 0.8) +
  facet_grid(context ~ ., scales = "free_y", switch = "both") +
  scale_color_manual(values = c("tomato1", "darkgreen", "dodgerblue2")) +
  labs(x = "% overlapped by genes", 
       y = "z-score",
       fill = NULL,
...
ggsave("z-meth vs gene.svg", height = 6, width = 4)
```
...
heatmap_data_s %>% 
  ...
  ggplot(aes(x = start, y = z)) +
  facet_grid(context + sample_type.f ~ chr, scales = "free_y", space = "free_x", switch = "both") +
  geom_bar(stat = "identity", aes(fill = sample_type.f), alpha = 0.8) +
  scale_fill_manual(values = c("grey20", "tomato1", "darkgreen", "dodgerblue2")) + 
  labs(x = NULL, 
       y = NULL,
