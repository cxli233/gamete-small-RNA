...
```
```{r}
h2.5 <- glmTreat(fit, coef=4, lfc=1)
h5 <- glmTreat(fit, coef=5, lfc=1)
h9 <- glmTreat(fit, coef=5, lfc=1)
m1 <- glmTreat(fit, coef=3, lfc=1)
ie <- glmTreat(fit, coef=2, lfc=1)
```
```{r}
de2 <- h2.5$table
de2$adjusted.p.value <- p.adjust(de2$PValue, method = "BH")
de2 <- de2 %>% mutate("sig"=ifelse(adjusted.p.value<0.05,"s","ns")) %>% mutate("gene.names" = row.names(h2.5$table)) %>% dplyr::select(gene.names, logFC, logCPM, adjusted.p.value, sig) 
de5 <- h5$table
de5$adjusted.p.value <- p.adjust(de5$PValue, method = "BH")
de5 <- de5 %>% mutate("sig"=ifelse(adjusted.p.value<0.05,"s","ns")) %>% mutate("gene.names" = row.names(h5$table)) %>% dplyr::select(gene.names, logFC, logCPM, adjusted.p.value, sig) 
de9 <- h9$table
de9$adjusted.p.value <- p.adjust(de9$PValue, method = "BH")
de9 <- de9 %>% mutate("sig"=ifelse(adjusted.p.value<0.05,"s","ns")) %>% mutate("gene.names" = row.names(h9$table)) %>% dplyr::select(gene.names, logFC, logCPM, adjusted.p.value, sig) 
dem1 <- m1$table
dem1 <- mutate(dem1, "adjusted.p.value" = p.adjust(PValue, method = "BH")) %>% mutate("sig"=ifelse(adjusted.p.value<0.05,"s","ns")) %>% mutate("gene.names" = row.names(m1$table)) %>% dplyr::select(gene.names, logFC, logCPM, adjusted.p.value, sig) 
deie <- ie$table
deie <- mutate(deie, "adjusted.p.value" = p.adjust(PValue, method = "BH")) %>% mutate("sig"=ifelse(adjusted.p.value<0.05,"s","ns")) %>% mutate("gene.names" = row.names(ie$table)) %>% dplyr::select(gene.names, logFC, logCPM, adjusted.p.value, sig) 
```
```{r}
ggplot(de2, aes(x = logCPM, y = logFC)) +
  geom_point(aes(color = sig, size = sig))+
  geom_line(y= 1, color = "blue")+
  geom_line(y = -1, color = "blue")+ 
...
ggplot(de5, aes(x = logCPM, y = logFC)) +
  geom_point(aes(color = sig, size = sig))+
  geom_line(y= 1, color = "blue")+
  geom_line(y = -1, color = "blue")+ 
  scale_color_manual(values = c("black","red"))+
...
ggplot(de9, aes(x = logCPM, y = logFC)) +
  geom_point(aes(color = sig, size = sig))+
  geom_line(y= 1, color = "blue")+
  geom_line(y = -1, color = "blue")+ 
  scale_color_manual(values = c("black","red"))+
...
ggplot(dem1, aes(x = logCPM, y = logFC)) +
  geom_point(aes(color = sig, size = sig))+
  geom_line(y= 1, color = "blue")+
  geom_line(y = -1, color = "blue")+ 
  scale_color_manual(values = c("black","red"))+
...
ggplot(deie, aes(x = logCPM, y = logFC)) +
  geom_point(aes(color = sig, size = sig))+
  geom_line(y= 1, color = "blue")+
  geom_line(y = -1, color = "blue")+ 
  scale_color_manual(values = c("black","red"))+
...
```
```{r}
write.csv(de2, "de2.csv")
write.csv(de5, "de5.csv")
write.csv(de9, "de9.csv")
```
#Candidate genes for the method paper
```{r}
marker_genes <- read_excel("C:/Users/cxli9/Desktop/Li/PBI/PBI299 (Sundar Lab)/smRNA project/method paper/marker_genes.xlsx") 
```
```{r}
TPM_marker <- TPM_long %>% 
  group_by(gene.names, sample_type) %>% 
  summarise(mean.TPM = mean(TPM)) %>% 
  inner_join(marker_genes, by = "gene.names") %>% 
  mutate(category = factor(category, levels = c("egg", "sperm", "Ohnishi et al. 2011"))) %>% 
  ungroup()
...
TPM_marker %>% 
  ...
  mutate(sample_type = factor(sample_type, 
                              levels = c("egg cell", "sperm cell", "pollen vegetative cell"))) %>%
  ggplot(aes(x = sample_type, y = name)) +
