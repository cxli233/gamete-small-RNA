...
ggsave("siRNA_in_24nt_loci.svg", width = 7, height = 5.5)
#ggsave("siRNA_in_24nt_loci.png", width = 7, height = 5.5)
```
...
ggsave("siRNA_in_24nt_loci_ddm1_drm2.svg", width = 9, height = 5.5)
```
#ddm1ab and drm2 siRNA in 24nt siRNA loci (not cell specific - inclusive)
```{r}
egg_m <- identify_samples(egg) %>% 
  mutate(bin = "egg\nloci") %>% 
  filter(sample_type == "leaf")
seedling_m <- identify_samples(seedling) %>% 
  mutate(bin = "seedling\nshoot\nloci") %>% 
  filter(sample_type == "leaf")
sperm_m <- identify_samples(sperm) %>% 
  mutate(bin = "sperm\nloci") %>% 
  filter(sample_type == "leaf")
```

```{r}
loci_m2 <- rbind(egg_m, seedling_m, sperm_m, intersection_m) %>% 
  inner_join(total_siRNA_length1 %>% 
               select(-library), by = c("sample_ID", "genotype", "sample_type", "lib.size", "total.siRNA")) %>% 
  select(-library) 
loci_m2
```
##normalized by specific lengths
```{r}
m_lengths2 <- loci_m2 %>% 
  mutate("20" = `20.x` / `20.y`) %>% 
  mutate("21" = `21.x` / `21.y`) %>% 
  ...
  mutate("24" = `24.x` / `24.y`) %>% 
  mutate("25" = `25.x` / `25.y`) %>% 
  select(sample_ID, genotype, lib.size, sample_type, bin, `20`, `21`, `22`, `23`, `24`, `25`) %>%
  gather("length", "proportion", 6:11) %>% 
  mutate(norm = "per specific length") 
...
m_tot2 <- loci_m2 %>% 
  ...
  mutate(proportion = counts / total.siRNA) %>% 
  mutate(length = substr(length, start = 0, stop = 2)) %>% 
  select(sample_ID, genotype, lib.size, sample_type, bin, length, proportion) %>% 
  mutate(norm = "per total siRNA")
...
rbind(m_lengths2, m_tot2) %>% 
  ...
  ggplot(aes(x = length, y = proportion )) +
  facet_grid(norm ~ bin.f, scales = "free_y", switch = "y") +
  stat_summary(geom = "line", fun.y = mean, aes(group = genotype, color = genotype), size = 1.5, alpha = 0.7) +
